...
export default ({ dataStore, deviceInterface }) => {
  ...
  Object.keys(dataStore.deviceAlarms).forEach(deviceId => {
    ...
    let watcher = watchList[deviceId]
    if (!watcher) {
      watcher = {
        dangerCount: 0,
        dangerStamp: 0,
    ...
    if (currentValue >= alarm.dangerZone && watcher.dangerCount < maxDangerCount) {
    ...
    else if (currentValue < alarm.dangerZone && watcher.dangerCount > 0) {
